...
public:
	MacroProcessor() {
        ...
        keyword_parameter_table.open("kptab.txt", ios::in);
        ...
        actual_parameter_table.open("aptab.txt", ios::out);
		...
		for (int i = MNT[name].MDTP - 1; i < MDT.size(); i++) {
			for (int j = 0; j < MDT[i].size() - 1; j++) {
				if (MDT[i][j] == "MEND") {
					...
					if (MDT[i][j].size() > 3 && isdigit(MDT[i][j][3])) {
    					int index = reference + MDT[i][j][3] - '0';
					}
					if (MDT[i][j].size()>=4 && MDT[i][j].at(0) == '(') {
